...
export default function App({ dataSource = mockDataSource, homeBrandId = 'memory-block', initialPage = 'entry', demoBrand, localOnly = false }: { dataSource?: WorldDataSource; homeBrandId?: string; initialPage?: 'entry' | 'intake' | 'matching'; demoBrand?: Brand; localOnly?: boolean }) {
  ...
  return <div className={`app-shell flow-shell page-${page} mode-${mode}`} data-character-style="facet">
    <header className="flow-header">
      <button className="brand-lockup flow-logo" onClick={() => setPage('entry')} aria-label="Brand Relations 首页"><img src="/vi/mark-black.svg" alt="" /><span>Brand Relations</span></button>
      {page === 'matching' ? <nav className="mode-switch" aria-label="匹配模式"><button title="Gravity 匹配" aria-label="Gravity 匹配" aria-pressed={mode === 'gravity'} onClick={() => setMode('gravity')}><Icon name="orbit" /></button><button title="抽卡匹配" aria-label="抽卡匹配" aria-pressed={mode === 'draw'} onClick={() => setMode('draw')}><Icon name="cards" /></button></nav> : null}
    ...
    {page === 'entry' ? <CharacterEntry brands={brands} example={home} onCreate={() => { setEditingBrand(Boolean(home.profile)); setPage('intake'); }} /> : page === 'intake' ? <BrandIntakePage localOnly={localOnly} initialBrand={editingBrand || localOnly ? home : undefined} onBack={() => setPage(editingBrand ? 'matching' : 'entry')} onEnter={enter} /> : page === 'detail' && partner ? <PartnerDetailPage home={home} partner={partner} relation={ownRelationMap.get(partner.id)} onClose={() => setPage('matching')} onContact={startInvitation} /> : page === 'next' && partner && invitations[invitationKey] ? <CollaborationInvitation localOnly={localOnly} key={invitationKey} home={home} partner={partner} invitation={invitations[invitationKey]} dispatch={dispatchInvitation} onBack={() => setPage('detail')} /> : mode === 'draw' ? <DrawPage onClose={() => setMode('gravity')} key={home.id} brands={brands} home={home} relations={ownRelationMap} onChoose={choosePartner} /> : <>
    ...
    <main className="workspace">
      ...
      <RelationInspector focus={focus} target={selected} relation={relationMap.get(selected.id)} count={brands.length} onMatch={selected.id !== home.id ? () => choosePartner(selected.id) : undefined} />
    </main>
