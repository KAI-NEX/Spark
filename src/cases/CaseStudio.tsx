...
function BrandSheet({ brand }: { brand: ExampleBrand }) {
  const fields = [['Identity · 品牌身份', brand.identity], ['Audience · 受众', brand.audience], ['Assets · 已有能力', brand.offers], ['Intent · 正在寻找', brand.intent], ['Execution · 执行条件', brand.execution], ['Constraints · 限制', brand.constraints]];
...
function OpportunityDetail({ item }: { item: Opportunity }) {
  ...
  return <section className="cs-detail" aria-labelledby={headingId}>
    <div className="cs-detail-top"><span className="cs-eyebrow">{members.length} BRANDS / 一个合作假设</span><span className={`cs-verdict ${item.partnerIds.includes('heavy-form') ? 'is-caution' : ''}`}>{item.verdict}</span></div>
    <div className="cs-opportunity">
      <div className="cs-stage"><div className="cs-stage-orbit" /><div className={`cs-team size-${members.length}`}>{members.map(brand => <Portrait key={brand.id} brand={brand} />)}</div><p>MEMORY BLOCK + {item.partnerIds.length} {item.partnerIds.length === 1 ? 'PARTNER' : 'PARTNERS'}</p></div>
      <div className="cs-story"><p className="cs-eyebrow">POSSIBLE TOGETHER</p><h2 id={headingId}>{item.title}</h2><p className="cs-subtitle">{item.subtitle}</p><p className="cs-why">{item.why}</p><ul>{item.roles.map(role => <li key={role}>{role}</li>)}</ul></div>
    </div>
    ...
    <div className="cs-next"><div><span className="cs-eyebrow">NEXT SMALL STEP</span><p>{item.next}</p></div><div className="cs-actions"><button onClick={() => setSaved(!saved)} aria-pressed={saved}>{saved ? '已收藏 · 本次会话' : '收藏这个方向'}</button><button className="cs-primary" onClick={() => setPreview(!preview)} aria-expanded={preview}>{preview ? '收起邀请草稿' : '预览合作邀请 ↗'}</button></div></div>
    ...
    <details className="cs-members"><summary>查看成员六因子与 Avatar 依据</summary>{members.map(b => <BrandSheet key={b.id} brand={b} />)}</details>
...
export default function CaseStudio() {
  ...
  function exportData() {
    const url = URL.createObjectURL(new Blob([JSON.stringify({ fictional: true, note: 'Memory Block 沿用原始示例，伙伴与合作条件均为虚构。Avatar 使用现有能力映射规则。', mainBrand: memory, partners, referenceCases }, null, 2)], { type: 'application/json' }));
  ...
  return <main className="cs-shell">
    <header className="cs-header"><a href="/" className="cs-logo"><span>✳</span> BRAND RELATIONS</a><span className="cs-edition">MEMORY BLOCK / CASEBOOK 01</span><div><button onClick={exportData}>下载案例数据 ↓</button><a href="/">打开 Gravity ↗</a></div></header>
    <section className="cs-hero"><div><p className="cs-eyebrow">FICTIONAL BRANDS, CONCRETE POSSIBILITIES</p><h1>从你的品牌出发，<br /><em>看看可以一起做什么。</em></h1><p>6 位虚构伙伴，4 个合作样例。先看形象，再看彼此的贡献。</p></div><aside className="cs-you"><Portrait brand={memory} name={false} /><div><span className="cs-eyebrow">YOU ARE</span><h2>Memory Block</h2><p>概念设计 · 产品设计 · 原型探索</p><small>正在寻找：材料、小批量制造与渠道</small></div></aside></section>
