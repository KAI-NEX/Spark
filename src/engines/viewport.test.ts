...
const lodRank: Record<LOD, number> = { full: 0, simple: 1, marker: 2, dormant: 3 };
...
describe('Viewport-driven LOD', () => {
  ...
  it('wakes a low-fit node when exploring its region, without modifying its gravity position', () => {
    ...
    expect(calculateViewportLOD(getNodeScreenPosition(position, camera, size), 22, camera, size)).toBe('simple');
  ...
  it('opens a partial world, then trades old detail for newly visible nodes', () => {
    ...
    const primary = [...initial.values()].filter(lod => lod === 'full' || lod === 'simple');
    expect(primary.length).toBeGreaterThanOrEqual(8);
    expect(primary.length).toBeLessThanOrEqual(15);
    ...
    expect(['simple', 'full']).toContain(explored.get(farNode.brand.id));
