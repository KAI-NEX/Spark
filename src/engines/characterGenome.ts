...
export function validateBrief(value: unknown): CompanyBrief {
  ...
  if (!result.name || result.offers.length < 8) throw new Error('请填写公司名称，以及至少 8 个字符的已有能力描述。');
  return result;
}
...
export function loadCustomBrands(): Brand[] {
  try {
    ...
    return value.slice(0, 40).flatMap(item => {
      try {
        const brief = validateBrief(item);
        if (typeof item.id !== 'string' || !item.id.startsWith('company-') || item.id.length > 90 || item.character?.version !== 1) return [];
        return [candidateBrand(brief, validateCandidate(item.character, brief, item.character.source === 'ai' ? 'ai' : 'local'), item.id)];
      } catch { return []; }
    });
  } catch { return []; }
}
