...
固定版本 / Pinned commit: `e42ee74da982d96cb7cfd3522fbf44f909eac164`

源码完整保留在 `integrations/collider/`，没有修改上游实现。主应用直接导入 `brand-collider-skills-design/src/server/runtime.ts` 和 `text-provider.ts`，并读取同仓库六个方法文件及其契约。没有新增 npm 运行依赖。

The upstream source is retained unchanged in `integrations/collider/`. The app imports its actual runtime and text provider, and reads its six method files/contracts. No additional npm runtime dependency is needed for this text integration.
